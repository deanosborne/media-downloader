...
app.get('/api/filesystem/browse', async (req, res) => {
    try {
        ...
        const isAllowed = allowedPaths.some((allowed) => {
            ...
            return (browsePath.startsWith(normalizedAllowed) ||
...
app.get('/api/tv/:id/season/:season/episode/:episode', async (req, res) => {
    try {
        const details = await getEpisodeDetails(req.params.id, req.params.season, req.params.episode);
        res.json(details);
    }
    catch (error) {
        res.status(500).json({ error: error.message });
...
app.get('/api/torrents/search', async (req, res) => {
    try {
        ...
        if (type === 'TV Show' && season) {
            searchQuery = buildTVSearchQuery(query, parseInt(season), episode ? parseInt(episode) : null);
        }
        const qualityPrefs = {
        ...
        if (type === 'TV Show' && season) {
            torrents = filterTVTorrents(torrents, parseInt(season), episode ? parseInt(episode) : null);
        }
